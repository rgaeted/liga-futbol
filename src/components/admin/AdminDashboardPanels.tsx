      ...
      {rows.map((m) => (
        ...
        >
          <div className="leading-tight">
            <div className="text-[13px] font-bold capitalize">{m.day}</div>
            <div className="text-xs text-zinc-400">{m.time}</div>
          </div>
          <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_58px_minmax(0,1fr)] items-center gap-2 max-sm:grid-cols-1 max-sm:gap-3">
            <div className="flex min-w-0 items-center justify-end gap-2 max-sm:justify-start">
              <span className="min-w-0 text-right text-sm font-semibold leading-snug max-sm:text-left">
                {m.home}
              </span>
              <span
            ...
            <div className="text-center font-display text-[21px] font-bold tracking-wide text-zinc-900 max-sm:order-first">
              {m.score}
            </div>
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="min-w-0 text-sm font-semibold leading-snug">{m.away}</span>
            </div>
          </div>
          <div className="flex w-[108px] shrink-0 flex-col items-end gap-1 max-lg:w-full max-lg:items-start">
            ...
            <span className="max-w-full text-right text-[11px] leading-snug text-zinc-400 max-lg:text-left">
              {m.venue}
            </span>
          </div>
        </Link>
...
export function AdminDashboardPanels({
  ...
  const tabClass = (active: boolean) =>
    active
      ? 'rounded-lg bg-white px-3.5 py-1.5 font-ui text-[13px] font-bold text-zinc-900 shadow-[0_1px_2px_rgba(24,24,27,0.12)]'
      : 'rounded-lg px-3.5 py-1.5 font-ui text-[13px] font-bold text-zinc-500'

  return (
    <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(0,1fr)]">
      <div className="flex flex-col gap-4">
        <section className="overflow-hidden rounded-[14px] border border-zinc-200 bg-white">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-zinc-100 px-5 py-4">
            <div className="flex gap-1.5 rounded-[10px] bg-zinc-100 p-1">
        ...
        <section>
          <div className="mb-3 flex items-baseline justify-between gap-3">
            <h2 className="font-display text-[22px] font-bold uppercase tracking-wide text-zinc-900">
              Accesos rápidos
            </h2>
            <span className="text-xs text-zinc-400">Administración del torneo</span>
          </div>
      ...
      <div className="flex flex-col gap-4">
        ...
        {scorers.length > 0 && (
          <section className="rounded-[14px] border border-zinc-200 bg-white px-5 pb-2 pt-4">
            <h2 className="mb-1 font-display text-xl font-bold uppercase tracking-wide text-zinc-900">
              Goleadores
            </h2>
            {scorers.map((p, index) => (
              <div
              ...
              >
                <div className="grid h-[30px] w-[30px] shrink-0 place-items-center rounded-full bg-zinc-900 text-[11px] font-bold text-white">
                  {p.abbr}
                </div>
                <div className="min-w-0 flex-1">
                  <div className="truncate text-[13px] font-semibold">{p.name}</div>
                  <div className="text-[11px] text-zinc-400">{p.team}</div>
                </div>
                <div className="font-display text-xl font-bold">{p.goals}</div>
              </div>
            ))}
          </section>
        )}

        <section className="rounded-[14px] border border-zinc-200 bg-white px-5 py-4">
          <h2 className="mb-3 font-display text-xl font-bold uppercase tracking-wide text-zinc-900">Pendientes</h2>
          <div className="flex flex-col gap-2.5">
            {todos.map((t, index) => (
              <div
                key={`${t.title}-${index}`}
                className="flex items-start gap-3 rounded-[10px] border border-zinc-100 bg-[#fafafa] px-3 py-2.5"
              >
                <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full" style={{ background: t.dot }} />
                <div className="min-w-0">
                  <div className="text-[13px] font-semibold text-pretty">{t.title}</div>
                  <div className="text-xs text-zinc-500">{t.meta}</div>
                </div>
              </div>
            ))}
