import { useCallback, useEffect, useRef, useState } from 'react'
import { AppState, type AppStateStatus } from 'react-native'
import type { MobileLiveSnapshot } from '@liga/mobile-contracts'
import { mobileApiClient } from '../api/client'
import { mobileApiPaths } from '../api/paths'
import { mobileLiveSnapshotSchema } from '@liga/mobile-contracts/schemas'
import { useMatchRealtime, type MatchRealtimeStatus } from './useMatchRealtime'

const INVALIDATION_DEBOUNCE_MS = 250
export const POLL_INTERVAL_MS = 15_000

export async function fetchMobileLiveSnapshot(
  matchId: string,
  signal?: AbortSignal,
): Promise<MobileLiveSnapshot> {
  const snapshot = await mobileApiClient.get(
    mobileApiPaths().live(matchId),
    mobileLiveSnapshotSchema,
    { signal },
  )
  if (snapshot.id !== matchId) throw new Error('live_snapshot_match_id_mismatch')
  return snapshot
}

export function useMobileLiveSnapshot({
  initialSnapshot,
}: {
  initialSnapshot: MobileLiveSnapshot
}) {
  const [snapshotState, setSnapshotState] = useState(() => ({
    initialSnapshot,
    snapshot: initialSnapshot,
  }))
  const debounceRef = useRef<ReturnType<typeof setTimeout> | null>(null)
  const abortRef = useRef<AbortController | null>(null)
  const requestRef = useRef(0)
  const previousStatusRef = useRef<MatchRealtimeStatus>('connecting')
  const lifecycleMatchIdRef = useRef(initialSnapshot.id)
  const initialSnapshotRef = useRef(initialSnapshot)
  const appStateRef = useRef<AppStateStatus>(AppState.currentState)
  const snapshot =
    snapshotState.initialSnapshot === initialSnapshot
      ? snapshotState.snapshot
      : initialSnapshot

  const resync = useCallback(async () => {
    const requestId = ++requestRef.current
    const requestSnapshot = initialSnapshotRef.current
    abortRef.current?.abort()
    const controller = new AbortController()
    abortRef.current = controller
    try {
      const next = await fetchMobileLiveSnapshot(requestSnapshot.id, controller.signal)
      if (requestId === requestRef.current) {
        setSnapshotState({ initialSnapshot: requestSnapshot, snapshot: next })
      }
    } catch {
      // Keep last valid snapshot visible.
    }
  }, [])

  const scheduleResync = useCallback(() => {
    if (debounceRef.current !== null) clearTimeout(debounceRef.current)
    debounceRef.current = setTimeout(() => {
      debounceRef.current = null
      void resync()
    }, INVALIDATION_DEBOUNCE_MS)
  }, [resync])

  useEffect(() => {
    const matchIdChanged = lifecycleMatchIdRef.current !== initialSnapshot.id
    lifecycleMatchIdRef.current = initialSnapshot.id
    initialSnapshotRef.current = initialSnapshot
    if (debounceRef.current !== null) {
      clearTimeout(debounceRef.current)
      debounceRef.current = null
    }
    ++requestRef.current
    abortRef.current?.abort()
    abortRef.current = null
    if (matchIdChanged) previousStatusRef.current = 'connecting'
  }, [initialSnapshot])

  const realtimeStatus = useMatchRealtime({
    matchId: initialSnapshot.id,
    enabled: true,
    onInvalidate: scheduleResync,
  })

  useEffect(() => {
    if (realtimeStatus === 'connected' && previousStatusRef.current !== 'connected') {
      void resync()
    }
    previousStatusRef.current = realtimeStatus
  }, [realtimeStatus, resync])

  useEffect(() => {
    if (snapshot.status !== 'LIVE' && snapshot.status !== 'HALFTIME') return

    const poll = () => {
      if (appStateRef.current === 'active') void resync()
    }

    const interval = setInterval(poll, POLL_INTERVAL_MS)
    const subscription = AppState.addEventListener('change', (nextState) => {
      const wasBackground = appStateRef.current.match(/inactive|background/)
      appStateRef.current = nextState
      if (wasBackground && nextState === 'active') {
        void resync()
      }
    })

    return () => {
      clearInterval(interval)
      subscription.remove()
    }
  }, [resync, snapshot.status])

  useEffect(
    () => () => {
      if (debounceRef.current !== null) clearTimeout(debounceRef.current)
      abortRef.current?.abort()
    },
    [],
  )

  return { snapshot, realtimeStatus, resync }
}
